...
| Published stack in a browser | ./e2e/run (Playwright, Chromium headless shell, the release stack from the local artifacts or, with E2E_BASE_URL, any running stack) | One host and one guest through a full round in a real browser through nginx: login, open room, deep-link reload, live WebSocket badge, join by PIN, start, answer, reveal, finish, archived history; NOT part of scripts/verify or CI, run by hand before a release; NOT load, slow-network or outage behavior |
