...
| Pull request shape | node scripts/check-pr.mjs (workflow pr-shape); self-test node --test scripts/check-pr.test.mjs (frontend-test stage) | PENDING first run: self-test PASS 8/8 locally; the titles and bodies of the merged pull requests 1, 3, 19, 22 and 26 pass the check | PENDING | macOS host, Node 22.22.3 | 2026-09-24 | PENDING |
