...
public class HistoryController {
    private final JdbcTemplate db;public HistoryController(JdbcTemplate db){this.db=db;}
    ...
    @GetMapping("/{id}") List<HistoryResult> result(HttpServletRequest r,@PathVariable UUID id){
        var rows=db.queryForList("select phase from game_room where id=? and owner_id=?",id,Identity.current(r).userId());
