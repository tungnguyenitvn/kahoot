...
public class QuizController {
    ...
    @GetMapping List<Quiz> list(HttpServletRequest r) { return catalog.list(Identity.current(r).userId()); }
    @PostMapping Quiz create(HttpServletRequest r, @Valid @RequestBody DraftRequest draft) { return catalog.create(Identity.current(r).userId(), draft.toDomain()); }
    @PostMapping("/{id}/publish") Quiz publish(HttpServletRequest r, @PathVariable UUID id) { return catalog.publish(Identity.current(r).userId(), id); }
}
